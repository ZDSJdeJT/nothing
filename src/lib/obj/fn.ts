...
export class Fn implements Obj {
  ...
  public inspect(): string {
    return `${TOKEN_LITERAL.FN}${TOKEN_LITERAL.OPEN_PAREN}${this.parameters.map((item) => item.string()).join(`${TOKEN_LITERAL.COMMA} `)}${TOKEN_LITERAL.CLOSE_PAREN} ${TOKEN_LITERAL.OPEN_BRACE}
  ${this.body.string()}
